...
An album summary contains `id`, `title`, `artwork_url`, `song_count`, and `latest_released_at`.

Album detail contains `id`, `title`, `artwork_url`, `released_at`, an `artists` array, and a `songs` array. Its `released_at` uses the manual album date when set, otherwise the latest release date among visible tracks. Each album artist contains `id`, `name`, `prefix_title`, and `is_group`.
