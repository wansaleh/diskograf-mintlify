...
- Loved-song endpoints return only published, non-deleted songs.
...
### Albums and curated lists

| Method and route | Description | Parameters |
| --- | --- | --- |
| `GET /v1/albums` | Lists published albums with at least one visible song. | No query parameters. |
| `GET /v1/albums/{albumId}` | Returns album metadata, artists, and ordered tracks. | `albumId` is a positive integer. |
| `GET /v1/songs/{songId}/albums` | Lists published album memberships for one public song. | `songId` is a positive integer. |
| `GET /v1/curated-lists/{listId}` | Returns one published editorial song list. | `listId` is a positive integer. |

An unknown album, song, or curated list returns `404`. Album and curated-list responses exclude unpublished and deleted songs.
...
| `GET /v1/users/{username}/loves` | Lists the member's published, non-deleted loved songs. | `limit`, `offset` |
...
Each credit contains `order`, an `artist` object, and a `role` object. A role can be `null` for an unclassified credit. Each artist includes a resolved `avatar_url`. Each role includes `id`, `title`, `slug`, `tag`, and `order`. Each genre contains `id`, `slug`, and `title`; each label contains `id` and `name`.

Song detail also includes `notes`, `lyrics`, and `love_count`. The `media` object contains named YouTube, Spotify, Apple Music, date-type, and duration fields. The `youtube` object contains duration, views, views-per-day, and last-fetched fields. Raw song and YouTube JSON are not returned.
...
- `aliases`, `biodata`, `country`, `place`, birth and death fields, job, and status.
- `alias_artists`, each with `id`, `name`, and `prefix_title`.
- `links` with `website`, `instagram`, `twitter`, `tiktok`, `youtube`, `facebook`, `spotify`, and `applemusic` keys.
- `roles`, each with `id`, `slug`, `title`, and a credit `count`.
- `genres`, each with `id`, `slug`, `title`, and a song `count`.
- `associated_acts`, each with `id`, `name`, `slug`, `username`, and `avatar_url`.
...
### Album and curated-list data

An album summary contains `id`, `title`, `album_type`, `artwork_url`, `song_count`, and `latest_released_at`.

Album detail contains `id`, `title`, `album_type`, `artwork_url`, `released_at`, an `artists` array, and a `songs` array. `album_type` is `album`, `ep`, or `single`. Its `released_at` uses the manual album date when set, otherwise the latest release date among visible tracks. Each album artist contains `id`, `name`, `prefix_title`, and `is_group`.

Each album track is a song card plus `position`, `date_type`, `duration_ms`, and `youtube_duration_seconds`. `duration_ms` is the manually-entered track length; `youtube_duration_seconds` is the auto-fetched YouTube length, used as a fallback when `duration_ms` is `null`.

A song's album membership (`/v1/songs/{songId}/albums`) contains `id`, `title`, `artwork_url`, and the track `position`.

A curated list contains `id`, `title`, `description`, and a `songs` array of song cards.
...
```text
username, name, avatar_url, created_at, is_fan, profile, artist
```

`profile` contains named biography, location, date, job, status, alias, and social-link fields.

`artist` is either `null` or an object containing the claimed artist's `id`, `name`, `slug`, `prefix_title`, `aliases`, `is_group`, `total_songs`, `total_credits`, `first_release_year`, `last_release_year`, `avatar_url`, and `associated_acts`.
...
`GET /v1/users/{username}/loves` returns published, non-deleted song cards in love order. Its list metadata reports the total visible songs.
